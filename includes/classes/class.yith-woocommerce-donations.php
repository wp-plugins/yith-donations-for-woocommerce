...
if( !class_exists( 'YITH_WC_Donations' ) ){
    ...
    class YITH_WC_Donations{
        ...
        public function __construct(){
            ...
            $this->_official_documentation  =   '';
            $this->_premium_landing_url     =   'http://yithemes.com/themes/plugins/yith-woocommerce-donations/';
            ...
            add_action( 'woocommerce_single_product_summary', array( $this, 'add_form_donation' ), 25 );
           ...
           // add_filter( 'woocommerce_add_cart_item', array( $this, 'set_donation_price' ), 20, 1 );
            add_filter( 'woocommerce_get_price', array( $this, 'set_donation_price' ), 10, 2 );

            /*Ajax call for add donation in cart*/
            if( !defined('DOING_AJAX') || !DOING_AJAX )
                add_action( 'wp_loaded', array( $this, 'ywcds_add_donation' ) );
            else
                add_action( 'init', array( $this, 'ywcds_add_donation' ) );
        ...
        public function add_free_frontend_style_script(){
            ...
            $yith_wcds_frontend_l10n    =   array(
                ...
                'thanks_message'    =>  $this->_messages['success']
              );
        ...
        public function init_ywds_plugin(){
            ...
            if( $donation_id    ==  -1){
                ...
                $args   =   array(
                    ...
                    'post_type'     =>  'product',
                    'post_status'   =>  'publish',
                    'post_title'    =>  __('YITh WooCommerce Donations', 'ywcds'),
                    'post_content'  =>  ''
                );
        ...
        public function add_form_donation(){
            ...
            echo yith_wcds_get_template( 'add-donation-form.php', $args, true );
        }
        ...
        public function is_donation_in_cart( ){
            ...
            if( WC()->cart->get_cart_contents_count() > 0 ){
                ...
                foreach( WC()->cart->get_cart() as $cart_item_key => $value ){
                    ...
                    $curr_product   =   $value['data'];

                    if( $this->_donation_id ==  $curr_product->id )
                        return true;
                }
            }
        ...
        public function add( $product_id, $amount ){
            if( !empty( $amount ) ){
                ...
                if( $amount!=null && $amount > 0) {
                    ...
                    $is_in_cart =   $this->is_donation_in_cart();

                    if( ! $is_in_cart ){

                        WC()->session->ywcds_amount = $amount;
                        WC()->cart->add_to_cart( $this->_donation_id );
                        return 'true';
                    }
                    else
                        return 'already';
        ...
        public function set_donation_price( $price, $product ){

            if( $product->id == $this->_donation_id ){
                if( isset( WC()->session->ywcds_amount ) )
                    $price= WC()->session->ywcds_amount;
            }
